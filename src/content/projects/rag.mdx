...
* **Visual Timeline Mobile Padding:** Refined the visual milestone timeline margins (`ml-2 md:ml-3`) and padding (`pl-5 md:pl-8`), and implemented dynamic timeline circle sizes (`-left-[27px] md:-left-[37px]`) on mobile viewports to prevent any text clipping.
* **Vercel Speed Insights Integration:** Installed and configured `@vercel/speed-insights` inside `RootLayout` (`layout.tsx`). This dynamically tracks real-user Core Web Vitals (LCP, FID, CLS) and real-time page speeds directly on Vercel's analytics dashboard.
